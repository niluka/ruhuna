...
Begin VB.Form frmBHTPayments 
   ...
   End
End
Attribute VB_Name = "frmBHTPayments"
...
'        If .State = 1 Then .Close
'        temSql = "Select count(IncomeBillID) as BillCount from tblIncomeBill where Completed = 1 and IsInwardPaymentBill = 1"
'        .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
...
'        !DisplayBillID = DisplayBillID
        ...
        DisplayBillID = NewBHTDisplayBillID(Val(txtBillID.Text))
...
Private Sub printBill()
    ...
    temDBID = DisplayBillID
    
    If SelectForm(cmbPaper.Text, Me.hwnd) = 1 Then
        temBillPoints = PrintThisBill(CStr(temDBID), cmbPaymentMethod.Text, MyBHT.FirstName, Format(Date, "dd MM yyyy"), Format(Time, "hh:mm AMPM"), "BHT Payment", "")
