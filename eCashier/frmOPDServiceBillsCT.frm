...
Begin VB.Form frmOPDServiceBillsCT 
   BackColor       =   &H0080C0FF&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "OPD Service Bills"
   ClientHeight    =   8310
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   12900
   ...
   End
End
Attribute VB_Name = "frmOPDServiceBillsCT"
...
Private Sub btnAdd_Click()
    If IsNumeric(txtOPDBillID.Text) = False Then txtOPDBillID.Text = NewOPDBillID(dtpDate.Value, dtpTime.Value)
    Dim rsTem As New ADODB.Recordset
       
'    If gridService.Rows >= 2 Then
'        MsgBox "You can't add more than one service for a OPD Service Bill"
'        Exit Sub
'    End If
   ...
   End If
    ...
    With rsTem
        ...
        If IsNumeric(txtEditID.Text) = True Then
            ...
            End If
        Else
            temSql = "Select * from tblPatientService"
            .Open temSql, cnnStores, adOpenStatic, adLockOptimistic
            .AddNew
        End If
        !OPDBillID = Val(txtOPDBillID.Text)
        ...
        If lblSpeciality1(n).Visible = True Then
            With rsTem
                ...
                Else
                    .AddNew
                    !UserID = UserID
                    !ForOPDBillID = Val(txtOPDBillID.Text)
...
Private Sub btnUpdate_Click()
    ...
    With rsTem
        If .State = 1 Then .Close
        temSql = "Select Count(IncomeBillID) as BillCount from tblIncomeBill where Completed = true AND IsOPDBill = True"
        .Open temSql, cnnStores, adOpenStatic, adLockOptimistic
        If .RecordCount > 0 Then
        ...
        End If
    End With
    txtDisplayBillID = DisplayBillID
    With rsTem
        If .State = 1 Then .Close
...
Private Sub printBill()
    ...
    If SelectForm(cmbPaper.Text, Me.hwnd) = 1 Then
        ...
        PrintingText FieldX, temY, ValueX, 0, RadiologyName, LeftAlign, MyFOnt
        
        MyFOnt.Bold = False
        MyFOnt.Size = 10
        temY = Printer.CurrentY
        PrintingText FieldX, temY, ValueX, 0, RadiologyDescreption, LeftAlign, MyFOnt
        ...
        MyFOnt.Size = 10
        temY = Printer.CurrentY
        PrintingText FieldX, temY, ValueX, 0, "OPD Bills", LeftAlign, MyFOnt
        
        MyFOnt.Size = 10
        
        Printer.Print
        PrintingText FieldX, temY, ValueX, 0, "Bill No : " & txtDisplayBillID.Text, LeftAlign, MyFOnt
        temY = Printer.CurrentY
        temY = Printer.CurrentY
        PrintingText FieldX, temY, ValueX, 0, "Patient : " & cmbTitle.Text & " " & cmbPatient.Text, LeftAlign, MyFOnt
        temY = Printer.CurrentY
        MyFOnt.Bold = True
        PrintingText FieldX, temY, ValueX, 0, "Payment Method : " & cmbPaymentMethod.Text, LeftAlign, MyFOnt
        MyFOnt.Bold = False
        temY = Printer.CurrentY
        PrintingText FieldX, temY, ValueX, 0, "Date : " & Format(Date, "dd MMMM yyyy"), LeftAlign, MyFOnt
        temY = Printer.CurrentY
...
Private Sub chkForeigner_Click()
    ...
    With rsTem
        ...
        temSql = "SELECT * " & _
                    ...
                    "WHERE Deleted = False AND OPDBillID = " & Val(txtOPDBillID.Text)
        .Open temSql, cnnStores, adOpenStatic, adLockOptimistic
        While .EOF = False
        ...
        Wend
        If .State = 1 Then .Close
        temSql = "Select * from tblProfessionalCharges where ForOPDBillID = " & Val(txtOPDBillID.Text)
        .Open temSql, cnnStores, adOpenStatic, adLockOptimistic
        While .EOF = False
            If chkForeigner.Value = 1 Then
                !Fee = !Fee * 2
...
Private Sub cmbCategory_Click(Area As Integer)
    If cmbCategory.Locked = True Then
        MsgBox "You can't change the category once Services are added. Only one service category can be added for one OPD service bill. If you want to add a new service category, finish this bill and start a new bill or delete the already entered services."
...
Private Sub cmbCategory_Change()
    ...
    With rsSC
        If .State = 1 Then .Close
        temSql = "Select * from tblServiceSubCategory where ForOPD = True AND ServiceCategoryID = " & Val(cmbCategory.BoundText)
        .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
        If .RecordCount > 0 Then
            cmbSC.Visible = True
            lblSC.Visible = True
...
Private Sub cmbSC_Change()
    ...
    With rsSPC
        ...
        temSql = "SELECT Top 7 tblSpeciality.Speciality, tblSpeciality.SpecialityID, tblServiceProfessionalCharges.Fee,  tblServiceProfessionalCharges.StaffID, tblServiceProfessionalCharges.ServiceProfessionalChargesID " & _
                    ...
                    "Where (((tblServiceProfessionalCharges.ServiceSubcategoryID) = " & Val(cmbSC.BoundText) & ") AND ((tblServiceProfessionalCharges.Deleted)=False ))" & _
        ...
        For n = 0 To PSCCount - 1
            ...
            temSql = "SELECT tblTitle.Title & tblStaff.Name as TitleStaff, tblStaff.StaffID FROM tblStaff LEFT JOIN tblTitle ON tblStaff.TitleID = tblTitle.TitleID Where SpecialityID = " & !SpecialityID & " ORDER BY tblTitle.Title, tblStaff.Name"
...
'    If IsNumeric(txtSerialNo.Text) = True Then Exit Sub
    ...
    With rsTem
        If .State = 1 Then .Close
        temSql = "Select Max(tblPatientService.SerialNo) as MaxSerialNo from tblPatientService Where Deleted = False AND ServiceDate = #" & Format(dtpAppDate.Value, "dd MMMM yyyy") & "# AND ServiceCategoryID = " & Val(cmbCategory.BoundText) & " and SECESSIONID  = " & Val(cmbSecession.BoundText)
        .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
...
Private Sub FillGrid()
    ...
    With rsTem
        ...
        temSql = "SELECT tblPatientService.PatientServiceID, tblPatientService.ServiceDate, tblServiceCategory.ServiceCategory, tblServiceSubcategory.ServiceSubcategory, tblPatientService.Comments, tblPatientService.Charge, tblServiceSecession.ServiceSecession, tblPatientService.SerialNo " & _
                    ...
                    "WHERE (((tblPatientService.Deleted)=False) AND ((tblPatientService.OPDBillID)<> 0)  AND ((tblPatientService.OPDBillID)=" & Val(txtOPDBillID.Text) & ")) " & _
