...
Begin VB.Form frmLabBillCancellation 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Reprint of Lab Bills"
   ClientHeight    =   8175
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   9825
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "frmLabBillCancellation.frx":0000
      ...
      TabPicture(1)   =   "frmLabBillCancellation.frx":001C
   ...
   End
End
Attribute VB_Name = "frmLabBillCancellation"
...
Private Sub btnCancelBill_Click()
    If IsNumeric(cmbPaymentMethod.BoundText) = False Then
        ...
        Exit Sub
    End If
    If ProfessionalFeePaidLab(Val(txtBillID.Text)) = True Then
    ...
    With rsTem
        If .State = 1 Then .Close
        temSql = "Select * from tblProfessionalCharges where ForLabBIllID = " & Val(txtBillID.Text)
        .Open temSql, cnnStores, adOpenStatic, adLockOptimistic
        If .RecordCount > 0 Then
            While .EOF = False
                !Cancelled = True
    ...
    frmLabBillCancellationSearch.FormatGrid
    frmLabBillCancellationSearch.FillGrid
    Unload Me
End Sub
...
Private Sub printBill()
    ...
    If SelectForm(cmbPaper.Text, Me.hwnd) = 1 Then
        temBillPoints = PrintThisBill(txtDisplayBillID.Text, cmbPaymentMethod.Text, txtPt.Text, Format(Date, "dd MM yyyy"), Format(Time, "hh:mm AMPM"), "Laboratory Bill Cancellation", "")
...
Private Sub FillGrid()
    ...
    With rsTem
        ...
        temSql = "SELECT tblPatientService.PatientServiceID, tblPatientService.ServiceDate, tblServiceCategory.ServiceCategory, tblServiceSubcategory.ServiceSubcategory, tblPatientService.Comments, tblPatientService.Charge " & _
                    ...
                    "WHERE (((tblPatientService.Deleted)=0)  AND ((tblPatientService.LABBillID)<>0) AND ((tblPatientService.LABBillID)=" & Val(txtBillID.Text) & ")) " & _
