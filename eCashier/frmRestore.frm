...
Private Sub bttnRestore_Click()
    ...
    DoEvents
    FSys.CopyFile eCashierDatabase, txtPath.Text & "\eCashi" & Format(Date, "ddmmyy") & ".mdb", True
    FSys.CopyFile Grid1.Text, eCashierDatabase, True
...
Private Sub ListDatabases()
    ...
    If FSys.FolderExists(txtPath.Text) = True Then
        ...
        For Each ThisFile In AllFiles
            If UCase(Left(ThisFile.Name, 6)) = UCase("eCashi") Then
                TemString1 = Mid(ThisFile.Name, 7, 2)
                TemString2 = Mid(ThisFile.Name, 9, 2)
                TemString3 = Mid(ThisFile.Name, 11, 2)
                If IsNumeric(TemString1) = True And IsNumeric(TemString2) = True And IsNumeric(TemString3) = True Then
                    NowROw = NowROw + 1
                    Grid1.Rows = NowROw + 1
                    Grid1.Row = NowROw
                    ...
                    Grid1.Text = Format(TemDate, LongDateFormat)
            End If
