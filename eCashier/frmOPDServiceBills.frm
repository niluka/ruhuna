...
Begin VB.Form frmOPDServiceBills 
   BackColor       =   &H0080FFFF&
   ...
   End
End
Attribute VB_Name = "frmOPDServiceBills"
...
Private Sub btnUpdate_Click()
    ...
    DisplayBillID = NewOPDDisplayBillID(Val(txtOPDBillID.Text))
    
'    With rsTem
'        If .State = 1 Then .Close
'        temSQL = "Select COUNT(IncomeBillID) as BillCount from tblIncomeBill where Completed = 1 AND IsOPDBill = 1"
'        .Open temSQL, cnnStores, adOpenStatic, adLockReadOnly
'        If .RecordCount > 0 Then
...
Private Sub printBill()
    ...
    If SelectForm(cmbPaper.Text, Me.hwnd) = 1 Then
        ...
        temBillPoints = PrintThisBill(txtDisplayBillID.Text, cmbPaymentMethod.Text, cmbTitle.Text & " " & cmbPatient.Text, Format(Date, "dd MM yyyy"), Format(Time, "hh:mm AMPM"), "OPD Services", "")
...
Private Sub printBillOld()
    ...
    If SelectForm(cmbPaper.Text, Me.hwnd) = 1 Then
        ...
        MyFOnt.Size = 12
        temY = Printer.CurrentY
        PrintingText FieldX, temY, ValueX, 0, HospitalName, leftAlign, MyFOnt
        
        MyFOnt.Bold = False
        MyFOnt.Size = 10
        temY = Printer.CurrentY
        PrintingText FieldX, temY, ValueX, 0, HospitalDescreption, leftAlign, MyFOnt
...
Private Sub chkForeigner_Click()
    ...
    With rsTem
        ...
        temSQL = "SELECT * " & _
                    ...
                    "WHERE Deleted = 0 AND OPDBillID = " & Val(txtOPDBillID.Text)
        .Open temSQL, cnnStores, adOpenStatic, adLockOptimistic
        If .RecordCount > 30 Then Exit Sub
...
Private Sub cmbCategory_Change()
    ...
    With rsSC
        If .State = 1 Then .Close
        temSQL = "Select * from tblServiceSubCategory where   Deleted = 0 AND ForOPD = 1 AND ServiceCategoryID = " & Val(cmbCategory.BoundText) & " Order By ServiceSubCategory"
        .Open temSQL, cnnStores, adOpenStatic, adLockReadOnly
        If .RecordCount > 0 Then
            cmbSC.Visible = True
            lblSC.Visible = True
...
Private Sub fillGrid()
    ...
    With rsTem
        ...
        temSQL = "SELECT tblPatientService.PatientServiceID, tblPatientService.ServiceDate, tblServiceCategory.ServiceCategory, tblServiceSubcategory.ServiceSubcategory, tblPatientService.Comments, tblPatientService.Charge, tblServiceSecession.ServiceSecession, tblPatientService.SerialNo " & _
                    ...
                    "WHERE (((tblPatientService.Deleted)=0) AND ((tblPatientService.OPDBillID)<> 0)  AND ((tblPatientService.OPDBillID)=" & Val(txtOPDBillID.Text) & ")) " & _
