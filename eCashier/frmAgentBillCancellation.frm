...
Begin VB.Form frmAgentBillCancellation 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Cancellation of Agent Bills"
   ClientHeight    =   8175
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   9825
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "frmAgentBillCancellation.frx":0000
      ...
      TabPicture(1)   =   "frmAgentBillCancellation.frx":001C
   ...
   End
End
Attribute VB_Name = "frmAgentBillCancellation"
...
Private Sub btnCancelBill_Click()
    ...
    If ProfessionalFeePaidOPD(Val(txtBillID.Text)) = True Then
...
'    With rsTem
'        If .State = 1 Then .Close
'        temSql = "Select * from tblProfessionalCharges where ForAgentBIllID = " & Val(txtBillID.Text)
'        .Open temSql, cnnStores, adOpenStatic, adLockOptimistic
'        If .RecordCount > 0 Then
'            While .EOF = False
'                !Cancelled = True
...
'    End With
    ...
    frmAgentBillCancellationSearch.FormatGrid
    frmAgentBillCancellationSearch.fillGrid
...
Private Sub printBill()
    ...
    If SelectForm(cmbPaper.Text, Me.hwnd) = 1 Then
        ...
        temBillPoints = PrintThisBill(txtDisplayBillID.Text, cmbPaymentMethod.Text, txtPt.Text, Format(Date, "dd MM yyyy"), Format(Time, "hh:mm AMPM"), "Agent Bill Cancellation", "")
...
Private Sub BillDetails()
    ...
    With rsTem
        If .State = 1 Then .Close
        temSql = "SELECT     TOP 100 PERCENT dbo.tblIncomeBill.*, dbo.tblIncomeBill.CompletedTime AS IBCompletedTime, dbo.tblIncomeBill.CompletedDate AS IBCompletedDate, dbo.tblPaymentMethod.PaymentMethod AS MyPM, dbo.tblIncomeBill.NetTotal AS Expr1, tblBookedUser.Name AS BName, tblCancelledUser.Name AS CName, dbo.tblIncomeBill.Cancelled AS IBCancelled, dbo.tblIncomeBill.CancelledDate AS IBCancelledDate, dbo.tblIncomeBill.CancelledTime AS IBCancelledTime, tblRefundMethod.PaymentMethod AS MyRPM, dbo.tblAgent.Agent " & _
                    "FROM         dbo.tblIncomeBill LEFT OUTER JOIN  dbo.tblAgent ON dbo.tblIncomeBill.AgentID = dbo.tblAgent.AgentID LEFT OUTER JOIN  dbo.tblPaymentMethod ON dbo.tblIncomeBill.PaymentMethodID = dbo.tblPaymentMethod.PaymentMethodID LEFT OUTER JOIN  dbo.tblStaff tblBookedUser ON dbo.tblIncomeBill.CompletedUserID = tblBookedUser.StaffID LEFT OUTER JOIN  dbo.tblStaff tblCancelledUser ON dbo.tblIncomeBill.CancelledUserID = tblCancelledUser.StaffID LEFT OUTER JOIN dbo.tblPaymentMethod tblRefundMethod ON dbo.tblIncomeBill.CancelledPaymentMethodID = tblRefundMethod.PaymentMethodID " & _
    ...
    With rsTem
        ...
        While .EOF = False
            ...
            temSql = "SELECT tblStaff.Name, tblProfessionalCharges.* " & _
                        ...
                        "WHERE (((tblProfessionalCharges.PatientServiceID)=" & !PatientServiceID & "))"
