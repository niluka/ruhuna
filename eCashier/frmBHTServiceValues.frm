...
Begin VB.Form frmBHTServiceValues 
   ...
   End
End
Attribute VB_Name = "frmBHTServiceValues"
...
Private Sub btnExcel_Click()
    GridToExcel gridService, Me.Caption, "From " & Format(dtpFrom.Value, "dd MMMM yyyy") & " to " & Format(dtpTo.Value, "dd MMMM yyyy")
End Sub
...
Private Sub FillGrid()
    ...
    txtOTotal.Text = Empty
    txtPTotal.Text = Empty
    txtCount.Text = Empty
    
    Dim temHTotal As Double
    Dim temOTotal As Double
...
'
        ...
        temSql = "SELECT Sum(tblPatientService.HospitalCharge) AS SumOfHCharge, Sum(tblPatientService.ProfessionalCharge) AS SumOfPCharge, Count(tblPatientService.Charge) AS CountOfCharge, tblServiceCategory.ServiceCategory, tblServiceSubcategory.ServiceSubcategory, tblServiceCategory.InwardSurcharge " & _
                    "FROM         dbo.tblPatientService LEFT OUTER JOIN dbo.tblBHT ON dbo.tblPatientService.BHTID = dbo.tblBHT.BHTID LEFT OUTER JOIN dbo.tblServiceCategory ON dbo.tblPatientService.ServiceCategoryID = dbo.tblServiceCategory.ServiceCategoryID LEFT OUTER JOIN dbo.tblServiceSubcategory ON dbo.tblPatientService.ServiceSubcategoryID = dbo.tblServiceSubcategory.ServiceSubcategoryID LEFT OUTER JOIN dbo.tblServiceSecession ON dbo.tblPatientService.SecessionID = dbo.tblServiceSecession.ServiceSecessionID " & _
                    "WHERE  dbo.tblBHT.IsBHT = 1 AND tblPatientService.ServiceDate Between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' And '" & Format(dtpTo.Value, "dd MMMM yyyy") & "' AND tblPatientService.Deleted = 0 AND tblPatientService.BHTID <> 0  "
        If IsNumeric(cmbCat.BoundText) = True Then
            temSql = temSql & "  AND tblPatientService.ServiceCategoryID = " & cmbCat.BoundText & " "
        End If
        ...
        If IsNumeric(cmbS.BoundText) = True Then
            temSql = temSql & "AND tblPatientService.ServiceSubcategoryID = " & cmbS.BoundText & " "
        End If
        temSql = temSql & " GROUP BY tblServiceCategory.ServiceCategory, tblServiceSubcategory.ServiceSubcategory, tblServiceCategory.InwardSurcharge ORDER BY tblServiceCategory.ServiceCategory, tblServiceSubcategory.ServiceSubcategory"
        ...
        While .EOF = False
            ...
            If IsNull(!InwardSurcharge) = False Then
                ...
                gridService.Col = 2
                gridService.Text = Format(!SumOfHCharge * 100 / (!InwardSurcharge + 100), "0.00")
                temHTotal = temHTotal + Format(!SumOfHCharge * 100 / (!InwardSurcharge + 100), "0.00")
                
                gridService.Col = 3
                gridService.Text = Format(!SumOfHCharge * !InwardSurcharge / (!InwardSurcharge + 100), "0.00")
                temOTotal = temOTotal + Format(!SumOfHCharge * !InwardSurcharge / (!InwardSurcharge + 100), "0.00")
            ...
            Else
                ...
                gridService.Col = 3
                gridService.Text = Format(0, "0.00")
                temOTotal = temOTotal + Format(0, "0.00")
    ...
    txtOTotal.Text = Format(temOTotal, "0.00")
