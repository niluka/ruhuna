...
Begin VB.Form frmBHTServicesOPDValueD 
   ...
   End
End
Attribute VB_Name = "frmBHTServicesOPDValueD"
...
Private Sub btnExcel_Click()
    GridToExcel gridService, "BHT Services OPD Value", "From " & Format(dtpFromDate.Value, "dd MMMM yyyy") & " to " & Format(dtpToDate.Value, "dd MMMM yyyy")
End Sub

Private Sub btnPrint_Click()
    Dim ThisReportFormat As PrintReport
    GetPrintDefaults ThisReportFormat
    GridPrint gridService, ThisReportFormat, "BHT Services OPD Value", "From " & Format(dtpFromDate.Value, "dd MMMM yyyy") & " to " & Format(dtpToDate.Value, "dd MMMM yyyy")
    Printer.EndDoc
End Sub
...
Private Sub FillGrid()
    ...
    With rsTem
        ...
        temSql = "SELECT dbo.tblBHT.BHT, dbo.tblPatientService.BHTID, dbo.tblPatientMainDetails.FirstName, dbo.tblPatientService.Charge, dbo.tblPatientService.Comments, dbo.tblPaymentMethod.PaymentMethod, dbo.tblPatientService.ServiceDate,  dbo.tblPatientService.ServiceDate, dbo.tblPatientService.ServiceCategoryID,  dbo.tblPatientService.ServiceSubcategoryID "
        ...
        While .EOF = False
            ...
            gridService.Col = 3
            gridService.Text = ServiceCategory(!ServiceCategoryID)
            
            gridService.Col = 4
            gridService.Text = ServiceSubcategory(!SErviceSubcategoryID)
            ...
            If !SErviceSubcategoryID <> 0 Then
                temCharge = SubCatCharge(!SErviceSubcategoryID)
                If temCharge = 0 Then
                    temCharge = !Charge
                End If
            ElseIf !ServiceCategoryID <> 0 Then
                temCharge = CatCharge(!ServiceCategoryID)
                If temCharge = 0 Then
                    temCharge = !Charge
                End If
            Else
            ...
            gridService.Text = Format(temCharge, "0.00")
            TotalCharge = TotalCharge + temCharge
...
Private Function CatCharge(ID As Long) As Double
    Dim rsTem As New ADODB.Recordset
    With rsTem
        If .State = 1 Then .Close
...
Private Function SubCatCharge(ID As Long) As Double
    Dim rsTem As New ADODB.Recordset
    With rsTem
        If .State = 1 Then .Close
