Attribute VB_Name = "ModulePaperSize"
Option Explicit
...
If RetVal = 0 Then
    If Err.LastDllError = 5 Then
        TemResponce = MsgBox("You do not have permissions to add a form to " & Printer.DeviceName, vbExclamation, "Access Denied!")
    Else
        TemResponce = MsgBox("Error: " & Err.Description)
    End If
    AddNewForm = "none"
Else
...
If OpenPrinter(PrinterName, PrinterHandle, 0&) Then
    ...
    If FormName = "Bud" Then
        ...
        If GetFormName(PrinterHandle, FormSize, FormName) = 0 Then
            ...
            AddNewForm PrinterHandle, FormSize, "Bud"
            If GetFormName(PrinterHandle, FormSize, FormName) = 0 Then
...
Public Sub PrintTest()
    ...
    Printer.Line (0, 0)-(Printer.ScaleWidth, Printer.ScaleHeight), , B
    Printer.NewPage
...
Public Function AddMyNewForm(PrinterHandle As Long, FormName As String, NewFormHeight As Long, NewFormWidth As Long, NewFormBottom As Long, NewFormTop As Long, NewFormLeft As Long, NewFormRight As Long) As String
    Dim TemResponce As Long
    Dim FI1 As sFORM_INFO_1
    Dim aFI1() As Byte
    Dim RetVal As Long
    With FI1
        ...
        With .ImageableArea
            .Left = NewFormLeft
            .Top = NewFormTop
            .Right = NewFormWidth - NewFormRight
            .Bottom = NewFormHeight - NewFormBottom
        End With
    End With
    ReDim aFI1(Len(FI1))
    Call CopyMemory(aFI1(0), FI1, Len(FI1))
    ...
    If RetVal = 0 Then
        If Err.LastDllError = 5 Then
            TemResponce = MsgBox("You do not have permissions to add a form to " & Printer.DeviceName, vbExclamation, "Access Denied!")
        Else
            TemResponce = MsgBox("The form could not add, Please check all the values you entered", vbCritical, "NOT added")
        End If
        AddMyNewForm = "none"
    Else
        AddMyNewForm = FI1.pName
    End If
End Function
