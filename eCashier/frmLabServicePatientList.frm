...
Begin VB.Form frmLabServicePatientList 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "View Lab Services"
   ClientHeight    =   8970
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   14055
   ...
   End
End
Attribute VB_Name = "frmLabServicePatientList"
...
Private Sub btnExcel_Click()
    GridToExcel gridService, "Lab Service Bills " & cmbCat.Text, "From " & Format(dtpFrom.Value, "dd MMMM yyyy") & " to " & Format(dtpTo.Value, "dd MMMM yyyy")
End Sub

Private Sub btnPrint_Click()
    Dim ThisPrintReportSettings As PrintReport
    GetPrintDefaults ThisPrintReportSettings
    ThisPrintReportSettings.ColSpace = 200
    
    'ThisPrintReportSettings.ReportPrintOrientation = Landscape
    GridPrint gridService, ThisPrintReportSettings, "Lab Services", "From " & Format(dtpFrom.Value, "dd MMMM yyyy") & " to " & Format(dtpTo.Value, "dd MMMM yyyy")
    Printer.EndDoc
...
Private Sub FillGrid()
    ...
    With rsTem
        ...
        temSql = "SELECT     TOP 100 PERCENT dbo.tblServiceCategory.ServiceCategory, dbo.tblServiceSubcategory.ServiceSubcategory, dbo.tblServiceSecession.ServiceSecession, dbo.tblPatientService.*, dbo.tblIncomeBill.Completed, dbo.tblIncomeBill.IncomeBillID AS Expr1, dbo.tblIncomeBill.DisplayBillID, dbo.tblPatientMainDetails.FirstName, dbo.tblIncomeBill.Cancelled, dbo.tblIncomeBill.CancelledTime " & _
                    "FROM dbo.tblPatientMainDetails RIGHT OUTER JOIN dbo.tblIncomeBill ON dbo.tblPatientMainDetails.PatientID = dbo.tblIncomeBill.PatientID RIGHT OUTER JOIN dbo.tblPatientService LEFT OUTER JOIN dbo.tblServiceCategory ON dbo.tblPatientService.ServiceCategoryID = dbo.tblServiceCategory.ServiceCategoryID LEFT OUTER JOIN dbo.tblServiceSubcategory ON dbo.tblPatientService.ServiceSubcategoryID = dbo.tblServiceSubcategory.ServiceSubcategoryID LEFT OUTER JOIN dbo.tblServiceSecession ON dbo.tblPatientService.SecessionID = dbo.tblServiceSecession.ServiceSecessionID ON  dbo.tblIncomeBill.IncomeBillID = dbo.tblPatientService.LabBillID " & _
                    "WHERE tblPatientService.ServiceDate Between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' And '" & Format(dtpTo.Value, "dd MMMM yyyy") & "' "
        
        'temSql = temSql & " AND tblPatientService.Deleted = 0 AND tblIncomeBill.Completed=1 AND tblIncomeBill.Cancelled = 0  "
        
        
        temSql = temSql & " AND tblPatientService.Deleted = 0 AND tblIncomeBill.Completed=1 "
        ...
        While .EOF = False
            ...
            gridService.Col = 0
            gridService.Text = !IncomeBillID
            
            gridService.Col = 1
            If IsNull(!DisplayBillID) = False Then gridService.Text = !DisplayBillID
            ...
            gridService.Col = 8
            gridService.Text = Format(!ProfessionalCharge, "0.00")
            ...
            If !Cancelled = True Then
                gridService.Col = 10
                gridService.Text = "cancelled on " & Format(!CancelledTime, "dd MMM yyyy - hh:mm AMPM")
            Else
                temTotal = temTotal + ![Charge]
                temHTotal = temHTotal + ![HospitalCharge]
                temPTotal = temPTotal + ![ProfessionalCharge]
            End If
            temCount = temCount + 1
            .MoveNext
        Wend
    ...
    If temTotal > 0 Then
        ...
        gridService.Col = 9
        gridService.Text = Format(temTotal, "0.00")
    End If
    ...
    gridService.ColWidth(3) = 0
    gridService.ColWidth(6) = 0
