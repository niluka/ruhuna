...
Begin VB.Form frmGSBBookeepingSummeryExtended 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "GSB Bookeeping Summmery (extended)"
   ClientHeight    =   10845
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   13230
   ...
   End
End
Attribute VB_Name = "frmGSBBookeepingSummeryExtended"
...
Private Sub FillGrid()
    ...
    With rsTem
        ...
        temSql = "SELECT dbo.tblBHT.*, dbo.tblPatientMainDetails.FirstName FROM dbo.tblBHT LEFT OUTER JOIN dbo.tblPatientMainDetails ON dbo.tblBHT.PatientID = dbo.tblPatientMainDetails.PatientID Where IsGSB = 1 AND DOD between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' And '" & Format(dtpTo.Value, "dd MMMM yyyy") & "' "
...
Private Sub FillBHTServices(BHTID As Long)
    ...
    With rsTem
        ...
        temSql = "SELECT tblServiceCategory.ServiceCategory, Sum(tblPatientService.Charge) AS SumOfCharge " & _
                    ...
                    "WHERE tblPatientService.Deleted = 0  AND tblPatientService.BHTID = " & BHTID & " And tblPatientService.ServiceSubcategoryID = 0 " & _
        ...
        If .State = 1 Then .Close
        temSql = "SELECT dbo.tblServiceSubcategory.ServiceSubcategory, Sum(tblPatientService.Charge) AS SumOfCharge " & _
                    "FROM (tblPatientService LEFT JOIN tblServiceCategory ON tblPatientService.ServiceCategoryID = tblServiceCategory.ServiceCategoryID) LEFT JOIN tblServiceSubcategory ON tblPatientService.ServiceSubcategoryID = tblServiceSubcategory.ServiceSubcategoryID " & _
                    "WHERE tblPatientService.Deleted = 0  AND tblPatientService.BHTID = " & BHTID & " And tblPatientService.ServiceSubcategoryID <> 0 " & _
                    "GROUP BY tblServiceSubcategory.ServiceSubcategory"
        .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
        While .EOF = False
            NewRow !ServiceSubcategory, !SumOfCharge
            .MoveNext
        Wend
...
Private Sub btnExcel_Click()
    GridToExcel gridSummery, "Book Keeping Summery For GSBs", "From " & Format(dtpFrom.Value, "dd MMMM yyyy") & " to " & Format(dtpTo.Value, "dd MMMM yyyy")
