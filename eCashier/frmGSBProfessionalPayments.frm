...
Begin VB.Form frmGSBProfessionalPayments 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Professional Fee Payments for Green Sheet Bills"
   ClientHeight    =   8250
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10965
   ...
   End
End
Attribute VB_Name = "frmGSBProfessionalPayments"
...
Private Sub btnCancelPayment_Click()
    ...
    For n = 0 To lstIDs.ListCount - 1
        ...
        End If
    Next
    If chkPrint.Value = 1 Then printBill (True)
...
Private Sub printBill(Optional CancelBill As Boolean)
...
'        NoX = temBillPoints.VX
        ...
        Printer.Print
        If CancelBill = True Then
            temY = Printer.CurrentY
            temText = "Cancellation of Payment"
...
Private Sub FillList()
    ...
    With rsTem
        If .State = 1 Then .Close
        temSql = "SELECT tblBHT.BHT, tblProfessionalCharges.* FROM tblProfessionalCharges LEFT JOIN tblBHT ON tblProfessionalCharges.ForBHTID = tblBHT.BHTID Where Fee > 0 AND Cancelled =  0   AND tblBHT.IsGSB = 1 "
        If optToPay.Value = True Then
