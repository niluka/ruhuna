...
Begin VB.Form frmGSBBillReprintSearch 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "GSB Bill Reprint"
   ClientHeight    =   8625
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10995
   ...
   Begin MSDataListLib.DataCombo cmbUser 
      ...
      Text            =   ""
   End
   Begin btButtonEx.ButtonEx btnToReprint 
      Height          =   495
      Left            =   8280
      TabIndex        =   8
      Top             =   8040
   ...
   End
End
Attribute VB_Name = "frmGSBBillReprintSearch"
...
Private Sub FillGrid()
    ...
    With rsTem
        ...
        While .EOF = False
            ...
            gridBill.Col = 2
            gridBill.Text = !CompletedTime
            
            gridBill.Col = 3
            gridBill.CellAlignment = 1
            gridBill.Text = Format(!BHT, "") & " - " & Format(!FirstName, "")
...
Private Sub btnToExcel_Click()
    GridToExcel gridBill, "Green Sheet Bills", Format(dtpDate.Value, "dd MMMM yyyy") & vbTab & cmbUser.Text & vbTab & cmbPaymentMethod.Text

End Sub

Private Sub btnToReprint_Click()
    Dim temBillID As Long
    temBillID = Val(gridBill.TextMatrix(gridBill.Row, 0))
    If temBillID <> 0 Then
        frmBHTBillReprint.txtBillID.Text = temBillID
        frmBHTBillReprint.Show
    Else
        MsgBox "Please select a bill to reprint"
        gridBill.SetFocus
        Exit Sub
    End If
...
Private Sub gridBill_DblClick()
    ...
    temBillID = Val(gridBill.TextMatrix(gridBill.Row, 0))
    If temBillID <> 0 Then
        frmGSBBillReprint.txtBillID.Text = temBillID
        frmGSBBillReprint.Show
        frmGSBBillReprint.ZOrder 0
        frmGSBBillReprint.Top = 0
        frmGSBBillReprint.Left = 0
    End If
End Sub
