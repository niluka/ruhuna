...
Begin VB.Form frmPharmacyBillCancellation 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Pharmacy Bill Cancellations"
   ClientHeight    =   7830
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10155
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "frmPharmacyBillCancellation.frx":0000
      ...
      Tab(0).Control(3)=   "cmbPaymentMethod"
      Tab(0).Control(3).Enabled=   0   'False
      Tab(0).Control(4)=   "txtPaymentMethod"
      Tab(0).Control(4).Enabled=   0   'False
      Tab(0).Control(5)=   "txtPayments"
      Tab(0).Control(5).Enabled=   0   'False
      Tab(0).ControlCount=   6
      TabCaption(1)   =   "Printing"
      TabPicture(1)   =   "frmPharmacyBillCancellation.frx":001C
   ...
   End
End
Attribute VB_Name = "frmPharmacyBillCancellation"
...
Private Sub btnPay_Click()
    ...
    With rsTem
        ...
        If .RecordCount > 0 Then
            ...
            !CancelledValue = Val(txtPayments.Text)
        ...
        .Open temSql, cnnStores, adOpenStatic, adLockOptimistic
            If .RecordCount > 0 Then
            !PaidCancelledAtCashier = True
            !PaidCancelledAtCashierDate = Date
            !PaidCancelledAtCashierTime = Now
            !PaidCancelledAtCashierUserID = UserID
            .Update
        End If
        .Close
...
Private Sub FillList()
    ...
    With rsTem
        ...
        If optToSettle.Value = True Then
            temSql = temSql & "Where (((tblSaleBill.PaidAtCashier) = 1) And ((tblSaleBill.PaidCancelledAtCashier) = 0 ) And ((tblSaleBill.CancelledDate) = '" & Format(dtpDate.Value, "dd MMMM yyyy") & "') And ((tblSaleBill.CancelledTime) > '" & dtpTime.Value & "') And ((tblSaleBill.PaymentMethodID) = " & Val(cmbPaymentMethod.BoundText) & ") AND ((tblSaleBill.Cancelled)=1))"
        Else
            temSql = temSql & "Where (((tblSaleBill.PaidAtCashier) = 1) And ((tblSaleBill.PaidCancelledAtCashier) = 1 ) And ((tblSaleBill.CancelledDate) = '" & Format(dtpDate.Value, "dd MMMM yyyy") & "') And ((tblSaleBill.CancelledTime) > '" & dtpTime.Value & "') And ((tblSaleBill.PaymentMethodID) = " & Val(cmbPaymentMethod.BoundText) & ") AND ((tblSaleBill.Cancelled)=1))"
        End If
        ...
        While .EOF = False
            ...
            temText = temText & Left(!CancelledTime & Space(7), 7) & vbTab
            temText = temText & Right(Space(12) & Format(!NetPrice, "0.00"), 12) & vbTab
            If !PaidAtCashier = True And !PaidCancelledAtCashier = False Then
                temText = temText & "Paid on " & Format(!PaidAtCashierDate, "dd MMM yyyy")
                lstPaid.AddItem "True"
            ElseIf !PaidAtCashier = True And !PaidCancelledAtCashier = True Then
                temText = temText & "Repaid on " & Format(!PaidCancelledAtCashierDate, "dd MMM yyyy")
...
Private Sub printBill()
    ...
    If SelectForm(cmbPaper.Text, Me.hwnd) = 1 Then
        temBillPoints = PrintThisBill(CStr(TxSaleBillID), cmbPaymentMethod.Text, "", Format(Date, "dd MM yyyy"), Format(Time, "hh:mm AMPM"), "Pharmacy Bill Cancellation", "")
