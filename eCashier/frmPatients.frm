...
Option Explicit
    ...
    Dim rsRace As New ADODB.Recordset
    Dim rsMariatel As New ADODB.Recordset
...
Private Sub FillCombos()
    ...
    With rsSex
        If .State = 1 Then .Close
        TemSql = "SELECT * from tblSex order by sex"
        .Open TemSql, cnnStores, adOpenStatic, adLockReadOnly
    End With
    With dtcSex
        Set .RowSource = rsSex
    ...
    With rsMariatel
        If .State = 1 Then .Close
        TemSql = "SELECT * from tblMarietal order by marietal"
        .Open TemSql, cnnStores, adOpenStatic, adLockReadOnly
    End With
    With dtcMarietal
        Set .RowSource = rsMariatel
        .ListField = "Marietal"
        .BoundColumn = "MarietalID"
    End With
    With rsRace
        If .State = 1 Then .Close
        TemSql = "SELECT * from tblRace order by Race"
        .Open TemSql, cnnStores, adOpenStatic, adLockReadOnly
    End With
    With dtcRace
        Set .RowSource = rsRace
        .ListField = "Race"
        .BoundColumn = "RaceID"
    End With
End Sub
...
End If
    ...
    For TemArrey1 = TemRow To TemRowSel
        ...
        If TemResponce = vbYes Then
            ...
            If .State = 1 Then .Close
            .Source = "select * from tblpatientmaindetails where PatientID = " & Grid1.TextMatrix(TemArrey1, 0)
            .Open
...
Private Sub FillPatientCombo()
    ...
    dtcPatient.BoundColumn = "PatientID"
...
With rsviewPatient
    If .State = 1 Then .Close
    TemSql = "SELECT * FROM tblPatientmainDetails where PatientID = " & Val(txtSearchID.Text) & " order by PatientID"
    .Open TemSql, cnnStores, adOpenStatic, adLockReadOnly
...
Private Sub ListBothNames()
    ...
    With rsviewPatient
        If .State = 1 Then .Close
        TemSql = "SELECT  PatientID, surname, firstname, (surname + firstname) as Temname FROM tblPatientmainDetails where (surname like '" & txtSearchSurname.Text & "%') and ( firstname like '" & txtSearchFirstName.Text & "%')order by FIrstname,surname"
        .Open TemSql, cnnStores, adOpenStatic, adLockReadOnly
        Set dtcPatient.RowSource = rsviewPatient
        dtcPatient.BoundColumn = "PatientID"
...
Private Sub GetDetails()
    ...
    With rsTem
        If .State = 1 Then .Close
        TemSql = "SELECT tblPatientmaindetails.* FROM tblPatientmainDetails where (PatientID =" & Val(dtcPatient.BoundText) & ")"
        .Open TemSql, cnnStores, adOpenStatic, adLockReadOnly
        If .RecordCount = 0 Then Exit Sub
        ...
        If Not IsNull(!MaritalID) Then dtcMarietal.BoundText = !MaritalID
        If Not IsNull(!RaceID) Then dtcRace.BoundText = !RaceID
...
Private Sub SaveDetails()
    With rsTem
        ...
        If IsNumeric(dtcMarietal.BoundText) Then !MaritalID = dtcMarietal.BoundText
        If IsNumeric(dtcRace.BoundText) Then !RaceID = dtcRace.BoundText
        !NICNo = txtNIC.Text
...
Private Sub EditDetails()
    ...
    With rsTem
        If .State = 1 Then .Close
        TemSql = "Select* from tblPatientMainDetails where PatientID =" & Val(dtcPatient.BoundText) & ""
        .Open TemSql, cnnStores, adOpenStatic, adLockOptimistic
