...
Begin VB.Form frmRBillCancellation 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Cancellation of Roentgents Bills"
   ClientHeight    =   8175
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   9825
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "frmRBillCancellation.frx":0000
      ...
      TabPicture(1)   =   "frmRBillCancellation.frx":001C
   ...
   End
End
Attribute VB_Name = "frmRBillCancellation"
...
Private Sub btnCancelBill_Click()
    If IsNumeric(cmbPaymentMethod.BoundText) = False Then
        ...
        Exit Sub
    End If
    If ProfessionalFeePaidR(Val(txtBillID.Text)) = True Then
    ...
    With rsTem
        If .State = 1 Then .Close
        temSql = "Select * from tblProfessionalCharges where ForRBIllID = " & Val(txtBillID.Text)
        .Open temSql, cnnStores, adOpenStatic, adLockOptimistic
        If .RecordCount > 0 Then
            While .EOF = False
                !Cancelled = True
    ...
    frmRBillCancellationSearch.FormatGrid
    frmRBillCancellationSearch.FillGrid
...
Private Sub printBill()
    ...
    If SelectForm(cmbPaper.Text, Me.hwnd) = 1 Then
        temBillPoints = PrintThisBill(txtDisplayBillID.Text, cmbPaymentMethod.Text, txtPt.Text, Format(Date, "dd MM yyyy"), Format(Time, "hh:mm AMPM"), "Roentgents Bill Cancellation", "")
...
Private Sub FillGrid()
    ...
    With rsTem
        ...
        temSql = "SELECT tblPatientService.PatientServiceID, tblPatientService.ServiceDate, tblServiceCategory.ServiceCategory, tblServiceSubcategory.ServiceSubcategory, tblPatientService.Comments, tblPatientService.HospitalCharge " & _
                    ...
                    "WHERE (((tblPatientService.Deleted)=0) AND ((tblPatientService.RBillID)<> 0)  AND ((tblPatientService.RBillID)=" & Val(txtBillID.Text) & ")) " & _
...
'                        "WHERE (((tblProfessionalCharges.ForRBillID)=" & Val(txtBillID.Text) & "))"
