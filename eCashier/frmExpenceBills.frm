...
Begin VB.Form frmExpenceBills 
   BackColor       =   &H00C0E0FF&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Expence Bills"
   ClientHeight    =   8310
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   12900
   ...
   End
End
Attribute VB_Name = "frmExpenceBills"
...
Private Sub btnAdd_Click()
    If IsNumeric(txtOPDBillID.Text) = False Then txtOPDBillID.Text = NewExpenceBillID(dtpDate.Value, dtpTime.Value)
    Dim rsTem As New ADODB.Recordset
       
'    If gridService.Rows >= 2 Then
'        MsgBox "You can't add more than one service for a Expence Service Bill"
'        Exit Sub
'    End If
   ...
   End If
    ...
    With rsTem
        ...
        Else
            ...
            .AddNew
        End If
        !ExpenceBillID = Val(txtOPDBillID.Text)
        ...
        If lblSpeciality1(n).Visible = True Then
            With rsTem
                ...
                Else
                    .AddNew
                    !UserID = UserID
                    !ForExpenceBillID = Val(txtOPDBillID.Text)
...
'    With rsTem
'        If .State = 1 Then .Close
'        temSQL = "Select Count(IncomeBillID) as BillCount from tblIncomeBill where Completed = 1 AND IsExpenceBill = 1"
'        .Open temSQL, cnnStores, adOpenStatic, adLockReadOnly
'        If .RecordCount > 0 Then
...
'    End With
'
    DisplayBillID = NewExpenceDisplayBillID(Val(txtOPDBillID.Text))
    ...
    With rsTem
        ...
        If .RecordCount > 0 Then
            ...
            !IsExpenceBill = True
            !GrossTotal = 0 - (Val(lblTotal.Caption))
            !NetTotal = 0 - (Val(lblTotal.Caption))
            !StoreID = UserStoreID
...
Private Sub printBill()
    ...
    If SelectForm(cmbPaper.Text, Me.hwnd) = 1 Then
        ...
        MyFOnt.Size = 10
        temY = Printer.CurrentY
        PrintingText FieldX, temY, ValueX, 0, "Expence Bills", leftAlign, MyFOnt
        ...
        temY = Printer.CurrentY
        temY = Printer.CurrentY
        PrintingText FieldX, temY, ValueX, 0, "Customer : " & cmbTitle.Text & " " & cmbPatient.Text, leftAlign, MyFOnt
        temY = Printer.CurrentY
        MyFOnt.Bold = True
...
Private Sub chkForeigner_Click()
    ...
    With rsTem
        ...
        temSQL = "SELECT * " & _
                    ...
                    "WHERE Deleted = 0 AND ExpenceBillID = " & Val(txtOPDBillID.Text)
        .Open temSQL, cnnStores, adOpenStatic, adLockOptimistic
        While .EOF = False
        ...
        Wend
        If .State = 1 Then .Close
        temSQL = "Select * from tblProfessionalCharges where ForExpenceBillID = " & Val(txtOPDBillID.Text)
        .Open temSQL, cnnStores, adOpenStatic, adLockOptimistic
        While .EOF = False
            If chkForeigner.Value = 1 Then
                !Fee = !Fee * 2
...
Private Sub cmbCategory_Change()
    ...
    With rsSC
        If .State = 1 Then .Close
        temSQL = "Select * from tblServiceSubCategory where   Deleted = 0 AND ForExpence = 1 AND ServiceCategoryID = " & Val(cmbCategory.BoundText)
        .Open temSQL, cnnStores, adOpenStatic, adLockReadOnly
        If .RecordCount > 0 Then
            cmbSC.Visible = True
            lblSC.Visible = True
...
Private Sub FillCombos()
    ...
    Cat.FillBoolCombo cmbCategory, "ServiceCategory", "ServiceCategory", "ForExpence", True
...
Private Sub fillGrid()
    ...
    With rsTem
        ...
        temSQL = "SELECT tblPatientService.PatientServiceID, tblPatientService.ServiceDate, tblServiceCategory.ServiceCategory, tblServiceSubcategory.ServiceSubcategory, tblPatientService.Comments, tblPatientService.Charge, tblServiceSecession.ServiceSecession, tblPatientService.SerialNo " & _
                    ...
                    "WHERE (((tblPatientService.Deleted)=0) AND ((tblPatientService.ExpenceBillID)<> 0)  AND ((tblPatientService.ExpenceBillID)=" & Val(txtOPDBillID.Text) & ")) " & _
