...
Begin VB.Form frmAgeAnalysisPatientBHT 
   ...
   End
End
Attribute VB_Name = "frmAgeAnalysisPatientBHT"
...
Private Sub btnToExcel_Click()
    ...
    GridToExcel gridBalance, "Age Analysis of BHT Patient Balance - " & cmbPaymentMethod.Text, "Date - " & Format(Date, LongDateFormat) & " - Time " & Format(Time, "HH MM AMPM")
...
'            gridBalance.ColWidth(6) = 1600
            ...
            gridBalance.Col = 4
            gridBalance.Text = "Bill No"
            
            gridBalance.Col = 5
            gridBalance.Text = "DOA"
            
            
            gridBalance.Col = 6
            gridBalance.Text = "DOD"
    ...
    With rsTem
        If IsNumeric(cmbPaymentMethod.BoundText) = True Then
            temSql = "SELECT tblPatientMainDetails.PatientID, tblPatientMainDetails.FirstName, tblBHT.* " & _
                        ...
                        "WHERE (((tblBHT.Discharge)=1) AND ((tblBHT.DOD) Between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & " 00:00:00 " & "' AND '" & Format(dtpTo.Value, "dd MMMM yyyy") & " 23:59:59 " & "' )  AND ((tblBHT.IsBHT)= 1 )  AND ((tblBHT.PaymentMethodID)=" & Val(cmbPaymentMethod.BoundText) & ") AND ((tblBHT.Balance)>0))" & _
                        "ORDER BY tblPatientMainDetails.FirstName"
        Else
            temSql = "SELECT tblPatientMainDetails.PatientID, tblPatientMainDetails.FirstName, tblBHT.* " & _
                        "FROM tblBHT LEFT JOIN tblPatientMainDetails ON tblBHT.PatientID = tblPatientMainDetails.PatientID " & _
                        "WHERE (((tblBHT.Discharge)=1) AND ((tblBHT.DOD) Between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & " 00:00:00 " & "' AND '" & Format(dtpTo.Value, "dd MMMM yyyy") & " 23:59:59 " & "' )  AND ((tblBHT.IsBHT)= 1 )  AND ((tblBHT.Balance)>0))" & _
                        "ORDER BY tblPatientMainDetails.FirstName"
        End If
        ...
        While .EOF = False
            ...
            gridBalance.Col = 4
            gridBalance.Text = MyBHT.BHTID
            
            gridBalance.Col = 5
            gridBalance.Text = Format(MyBHT.DOA, "dd MMM yyyy")
            
            gridBalance.Col = 6
            gridBalance.Text = Format(MyBHT.DOD, "dd MMMM yyyy")
