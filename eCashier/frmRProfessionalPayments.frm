...
Begin VB.Form frmRProfessionalPayments 
   ...
   End
End
Attribute VB_Name = "frmRProfessionalPayments"
...
Private Sub FillList()
    ...
    With rsTem
        If .State = 1 Then .Close
        temSql = "SELECT tblProfessionalCharges.*, tblServiceCategory.ServiceCategory, tblServiceSubCategory.ServiceSubCategory, tblProfessionalCharges.PatientServiceCharge, tblIncomeBill.IncomeBillID, tblIncomeBill.DisplayBillID , tblIncomeBill.PatientID "
        temSql = temSql & "FROM (((tblPatientService RIGHT JOIN tblProfessionalCharges ON tblPatientService.PatientServiceID = tblProfessionalCharges.PatientServiceID) LEFT JOIN tblServiceCategory ON tblPatientService.ServiceCategoryID = tblServiceCategory.ServiceCategoryID) LEFT JOIN tblServiceSubcategory ON tblPatientService.ServiceSubcategoryID = tblServiceSubcategory.ServiceSubcategoryID) LEFT JOIN tblIncomeBill ON tblPatientService.RBillID = tblIncomeBill.IncomeBillID "
        ...
        temSql = temSql & " AND ForRBillID <> 0 "
        ...
        While .EOF = False
            ...
            temText = temText & Left(FullStaffName(![StaffID]) & Space(37), 34)
            temText = temText & " (" & Left(FullPatientName(![PatientID]) & Space(17), 24) & ")"
