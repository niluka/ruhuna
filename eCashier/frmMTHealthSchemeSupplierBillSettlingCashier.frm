...
Begin VB.Form frmMTHealthSchemeSupplierBillSettlingCashier 
   Caption         =   "Medical Test Bill Settling by Health Scheme Suppliers"
   ClientHeight    =   9405
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   12660
   ...
   End
End
Attribute VB_Name = "frmMTHealthSchemeSupplierBillSettlingCashier"
...
Private Sub btnExcel_Click()
    GridToExcel gridBill, "Medical Test Bills"
End Sub

Private Sub btnUpdate_Click()
    Call GridBill_LeaveCell
    ...
    Else
        For n = 1 To gridBill.Rows - 1
            If Val(gridBill.TextMatrix(n, 10)) <> 0 Then
                ...
                UpdateBHTSettle Val(gridBill.TextMatrix(n, 5)), Val(gridBill.TextMatrix(n, 10)), True, False
                DisplayBillID = UpdateIncomeBill(Val(gridBill.TextMatrix(n, 12)), Val(gridBill.TextMatrix(n, 10)), True, False, Val(gridBill.TextMatrix(n, 5)))
                'If chkPrint.Value = 1 Then
                    MyPt.ID = Val(gridBill.TextMatrix(n, 11))
                    printBill gridBill.TextMatrix(n, 1), MyPt.FirstName, gridBill.TextMatrix(n, 10), Val(gridBill.TextMatrix(n, 10)), DisplayBillID
               ' End If
            End If
        Next
...
Private Sub UpdateBHTSettle(BHTID As Long, UpdateValue As Double, AddToSettle As Boolean, DeductFromSettle As Boolean)
    ...
    With rsRefill
        ...
        temSql = "Select * from tblIncomeBill where IncomeBillID = " & BHTID
...
Private Sub dtcPayment_GotFocus()
    GridBill_LeaveCell
End Sub
...
Private Sub gridBill_DblClick()
    Dim MTBILLID As Long
    With gridBill
        If IsNumeric(.TextMatrix(.Row, 5)) = True Then
            MTBILLID = Val(.TextMatrix(.Row, 5))
            Unload frmMTBillsView
            frmMTBillsView.Show
            frmMTBillsView.ZOrder 0
            frmMTBillsView.txtMedicalTestBillID.Text = MTBILLID
        End If
    End With
End Sub

Private Sub txtCellText_Change()
...
Private Function UpdateIncomeBill(HSSID As Long, UpdateValue As Double, AddToSettle As Boolean, DeductFromSettle As Boolean, MTBILLID As Long) As Long
    Dim rsRefill As New ADODB.Recordset
    Dim DisplayBillID As Long
    With rsRefill
'        If .State = 1 Then .Close
...
Private Sub GridBill_EnterCell()
    ...
    If gridBill.Col <> 10 Then Exit Sub
...
Private Sub FillGrid()
    ...
    With gridBill
        ...
        With rsBill
            temSql = "SELECT tblHealthSchemeSuppliers.HealthSchemeSupplierName, tblHealthSchemeSuppliers.HealthSchemeSupplierID, tblIncomeBill.DisplayBillID, tblIncomeBill.CompletedTime, tblIncomeBill.CompletedDate, tblIncomeBill.NetTotal, tblIncomeBill.PatientID, tblIncomeBill.Balance, tblIncomeBill.IncomeBillID, tblIncomeBill.PaymentComments, dbo.tblPatientMainDetails.FirstName  "
'            temSql = temSql & "FROM tblIncomeBill                                                                                                                  LEFT JOIN dbo.tblHealthSchemeSuppliers ON tblIncomeBill.HSSID = tblHealthSchemeSuppliers.HealthSchemeSupplierID "
            temSql = temSql & "FROM tblIncomeBill LEFT OUTER JOIN tblPatientMainDetails ON dbo.tblIncomeBill.PatientID = dbo.tblPatientMainDetails.PatientID LEFT OUTER JOIN dbo.tblHealthSchemeSuppliers ON dbo.tblIncomeBill.HSSID = dbo.tblHealthSchemeSuppliers.HealthSchemeSupplierID "

            temSql = temSql & "WHERE tblIncomeBill.Cancelled = 0  AND IsMedicalTestBill = 1 AND tblIncomeBill.CompletedDate Between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' And '" & Format(dtpTo.Value, "dd MMMM yyyy") & "' "
            If IsNumeric(dtcSupplier.BoundText) = True Then
            ...
            If IsNumeric(dtcPayment.BoundText) = True Then
                temSql = temSql & " AND tblIncomeBill.PaymentMethodID = " & dtcPayment.BoundText & " "
            End If
            If .State = 1 Then .Close
            .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
            If .RecordCount > 0 Then
...
'                    If Not IsNull(!HealthSchemeSupplierName) Then gridBill.TextMatrix(i, 1) = !HealthSchemeSupplierName
                    ...
                    If IsNumeric(dtcSupplier.BoundText) = True Then
                        If Not IsNull(!FirstName) Then gridBill.TextMatrix(i, 1) = !FirstName
                    Else
                        If Not IsNull(!FirstName) Then gridBill.TextMatrix(i, 1) = gridBill.TextMatrix(i, 1) & " - " & !FirstName
                    End If
                    
                    
                    If Not IsNull(!DisplayBillID) Then gridBill.TextMatrix(i, 2) = !DisplayBillID
                    If Not IsNull(!CompletedDate) Then gridBill.TextMatrix(i, 3) = Format(!CompletedDate, "dd MMMM yyyy")
                    If Not IsNull(!PaymentComments) Then gridBill.TextMatrix(i, 4) = Format(!PaymentComments, "")
                    
                    If Not IsNull(!IncomeBillID) Then gridBill.TextMatrix(i, 5) = !IncomeBillID

                    If Not IsNull(!CompletedDate) Then gridBill.TextMatrix(i, 6) = DateDiff("d", !CompletedDate, Date) & " days"

                    gridBill.TextMatrix(i, 7) = Format(!NetTotal, "#,##0.00")
                    TotalValue = TotalValue + !NetTotal
                    
                    gridBill.TextMatrix(i, 8) = Format(!NetTotal - Val(Format(!Balance, "0.00")), "#,##0.00")
                    
                    gridBill.TextMatrix(i, 9) = Format(!Balance, "#,##0.00")
                    
                    SettledValue = SettledValue + !NetTotal - Val(Format(!Balance, "0.00"))
                    ToSettleValue = ToSettleValue + Val(Format(!Balance, "0.00"))

                    If Not IsNull(!IncomeBillID) Then gridBill.TextMatrix(i, 11) = !PatientID
...
Private Sub printBill(CompanyName As String, PatientName As String, BHT As String, PaymentValue As Double, BillNo As Long)
    ...
    If SelectForm(cmbPaper.Text, Me.hwnd) = 1 Then
        temBillPoints = PrintThisBill(CStr(BillNo), cmbPaymentMethod.Text, PatientName, Format(Date, "dd MM yyyy"), Format(Time, "hh:mm AMPM"), "Company Payment", "")
...
'        NoX = temBillPoints.VX
        ...
        temY = Printer.CurrentY
        PrintingText FieldX, temY, ValueX, 0, "Medical Test Bill : " & BHT, leftAlign, MyFOnt
