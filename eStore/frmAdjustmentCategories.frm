...
Begin VB.Form frmAdjustmentCategories 
   ...
   End
End
Attribute VB_Name = "frmAdjustmentCategories"
...
Private Sub bttnChange_Click()
    ...
    On Error Resume Next
        If .State = 1 Then .Close
        .Open "Select * From tblAdjustmentCategory Where (AdjustmentCategoryID = " & TemCategoryID & ")", cnnStores, 3, 3
        If .RecordCount > 0 Then
            !AdjustmentCategory = txtCategoryName.Text
...
Private Sub bttnSave_Click()
    ...
    On Error Resume Next
        If .State = 1 Then .Close
        .Open "Select * From tblAdjustmentCategory", cnnStores, adOpenStatic, adLockOptimistic
        .AddNew
        !AdjustmentCategory = txtCategoryName.Text
        !Comments = txtComments.Text
        .Update
...
Private Sub NoName()
    Dim TemResponce As Integer
    TemResponce = MsgBox("You must enter a Adjustment Category name to save", vbCritical, "No Name")
    txtCategoryName.SetFocus
End Sub
...
Private Sub DisplaySelected()
    ...
    With rsCategory
        If .State = 1 Then .Close
        .Open "Select tblAdjustmentCategory.* From tblAdjustmentCategory Where (AdjustmentCategoryID = " & dtcCategoryName.BoundText & ")", cnnStores, adOpenStatic, adLockReadOnly
        If .RecordCount > 0 Then
            Call ClearValues
            If Not (!AdjustmentCategory) = "" Then txtCategoryName.Text = !AdjustmentCategory
            If Not (!Comments) = "" Then txtComments.Text = !Comments
            TemCategoryID = !AdjustmentCategoryID
        Else
            Call ClearValues
            txtCategoryName.Text = Empty
            txtComments.Text = Empty
