...
Begin VB.Form frmShiftEndCreditCardSale 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Shift End Credit Card"
   ClientHeight    =   6750
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   7620
   ...
   End
End
Attribute VB_Name = "frmShiftEndCreditCardSale"
...
Private Sub SelectPrint()
    With rsReport
        ...
        temSql = "SELECT tblSaleBill.SaleBillID, tblSaleBill.NetCost, tblSaleBill.NetPrice, tblSaleBill.Time, tblSaleBill.Date, tblStaff.Name, tblSaleBill.NetPrice " & _
                    ...
                    "WHERE (((tblSaleBill.Date) Between '" & Format(dtpFrom.Value, "dd MMMM yyyy") & "' And '" & Format(dtpTo.Value, "dd MMMM yyyy") & "') AND ((tblPaymentMethod.PaymentMethod)='Credit Card')  AND ((tblSaleBill.StaffID)= " & dtcStaff.BoundText & " ) )"
        .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
    End With
    With dtrShiftEndCashSale
        ...
        .Sections("Section4").Controls.Item("lblTopic").Caption = "Shift End Credit Card Sale - " & dtcStaff.Text
...
Private Sub FillGrid()
    ...
    With rsSale
        ...
        temSql = "SELECT tblSaleBill.SaleBillID, tblSaleBill.NetCost, tblSaleBill.Date, tblSaleBill.Time, tblStaff.Name, tblSaleBill.NetPrice " & _
                    "FROM (tblSaleBill LEFT JOIN tblStaff ON tblSaleBill.StaffID = tblStaff.StaffID) LEFT JOIN tblPaymentMethod ON tblSaleBill.PaymentMethodID = tblPaymentMethod.PaymentMethodID " & _
                    "WHERE (((tblSaleBill.Date) Between '" & dtpFrom.Value & "' And '" & dtpTo.Value & "') AND ((tblPaymentMethod.PaymentMethod)='Credit Card')  AND ((tblSaleBill.StaffID)=" & dtcStaff.BoundText & ") )"
        .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
