...
Begin VB.Form frmAddNewGenericName 
   ...
   End
End
Attribute VB_Name = "frmAddNewGenericName"
...
Private Sub bttnChange_Click()
    ...
    With rsIx
        ...
        .Open "SELECT * from tblIx where IX_ID = " & dtcIx.BoundText, cnnStores, adOpenStatic, adLockOptimistic
...
Private Sub bttnSave_Click()
    ...
    With rsIx
        If .State = 1 Then .Close
        .Open "SELECT * from tblIx where IX = '" & Trim(txtIx.Text) & "'", cnnStores, adOpenStatic, adLockOptimistic
        If .RecordCount > 0 Then
        ...
        End If
        .Close
        .Open "SELECT * from tblix", cnnStores, adOpenDynamic, adLockOptimistic
        .AddNew
        !ix = Trim(txtIx.Text)
        !ixcatogery_ID = dtcIxCatogery.BoundText
        If IsNumeric(dtcIxSubcatogery.BoundText) Then !ixsubcatogery_ID = dtcIxSubcatogery.BoundText
...
Private Sub GetIxDetails()
    ...
    With rsGetIxDetails
        If .State = 1 Then .Close
        .Open "SELECT * from tblix where Ix_ID = " & dtcIx.BoundText, cnnStores, adOpenStatic, adLockPessimistic
        txtIx.Text = !ix
...
Private Sub FillIxCombo()
    With rsIxList
        If .State = 1 Then .Close
        .Open "select * from tblix order by ix", cnnStores
