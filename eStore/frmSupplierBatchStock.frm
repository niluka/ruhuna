...
Begin VB.Form frmSupplierBatchStock 
   ...
   End
End
Attribute VB_Name = "frmSupplierBatchStock"
...
Private Sub btnExcel_Click()
    GridToExcel GridStock, "Category Details", cmbDistributor.Text
End Sub
...
Private Sub FillGrid()
    ...
    With rsBatchStock
        ...
        temSelect = "SELECT tblItem.Display, tblCurrentPurchasePrice.PPrice, tblItem.ItemID, min([tblCurrentPurchasePrice].[CurrentPPriceID])*Sum([tblBatchStock].[Stock]) AS StockValue, Sum(tblBatchStock.Stock) AS SumOfStock "
        temFrom = "FROM ((tblItem LEFT JOIN (tblBatchStock RIGHT JOIN tblBatch ON tblBatchStock.BatchID = tblBatch.BatchID) ON tblItem.ItemID = tblBatch.ItemID) LEFT JOIN tblCurrentPurchasePrice ON tblItem.ItemID = tblCurrentPurchasePrice.ItemID) RIGHT JOIN tblItemDistributor ON tblItem.ItemID = tblItemDistributor.ItemID "
        If IsNumeric(cmbDistributor.BoundText) = False Then
            temWhere = " "
        Else
            temWhere = "WHERE tblItemDistributor.DistributorID = " & Val(cmbDistributor.BoundText) & " "
        End If
...
Private Sub bttnPrint_Click()
    ...
    GridPrint GridStock, ThisReportFormat, "Supplier-vice Batch Stock - " & cmbDistributor.Text, Format(Date, LongDateFormat)
