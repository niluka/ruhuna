...
Begin VB.Form frmIncomeCategories 
   ...
   End
End
Attribute VB_Name = "frmIncomeCategories"
...
Private Sub FillCategoryName()
    With rsViewCategory
        ...
        .Open "Select* From tblIncomeCategory Order By IncomeCategory", cnnStores, adOpenStatic, adLockReadOnly
        If .RecordCount > 0 Then
            Set dtcCategoryName.RowSource = rsViewCategory
            dtcCategoryName.BoundColumn = "IncomeCategoryID"
            dtcCategoryName.ListField = "IncomeCategory"
        End If
    End With
End Sub
...
Private Sub bttnChange_Click()
    ...
    On Error Resume Next
        If .State = 1 Then .Close
        .Open "Select tblIncomeCategory.* From tblIncomeCategory Where (IncomeCategoryID = " & TemCategoryID & ")", cnnStores, 3, 3
        If .RecordCount > 0 Then
            !IncomeCategory = txtCategoryName.Text
...
Private Sub bttnSave_Click()
    ...
    On Error Resume Next
        If .State = 1 Then .Close
        .Open "Select tblIncomeCategory.* From tblIncomeCategory", cnnStores, adOpenStatic, adLockOptimistic
        .AddNew
        !IncomeCategory = txtCategoryName.Text
        !Comments = txtComments.Text
        .Update
...
Private Sub NoName()
    Dim TemResponce As Integer
    TemResponce = MsgBox("You must enter a Income Category name to save", vbCritical, "No Name")
    txtCategoryName.SetFocus
End Sub
...
Private Sub DisplaySelected()
    ...
    With rsCategory
        If .State = 1 Then .Close
        .Open "Select tblIncomeCategory.* From tblIncomeCategory Where (IncomeCategoryID = " & dtcCategoryName.BoundText & ")", cnnStores, adOpenStatic, adLockReadOnly
        Call ClearValues
        If .RecordCount > 0 Then
            If Not (!IncomeCategory) = "" Then txtCategoryName.Text = !IncomeCategory
            If Not (!Comments) = "" Then txtComments.Text = !Comments
            TemCategoryID = !IncomeCategoryID
        Else
            txtCategoryName.Text = Empty
            txtComments.Text = Empty
            TemCategoryID = 0
