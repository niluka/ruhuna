...
Begin VB.Form frmDiscard 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabHeight       =   520
      TabCaption(0)   =   "Item"
      TabPicture(0)   =   "frmDiscard.frx":0000
      ...
      Tab(0).Control(8)=   "txtVMPP"
      Tab(0).Control(9)=   "txtAMP"
      Tab(0).Control(10)=   "txtVMP"
      Tab(0).Control(11)=   "txtVTM"
      Tab(0).Control(12)=   "txtDisplay"
      Tab(0).Control(13)=   "txtIStore"
      Tab(0).Control(14)=   "txtRStoreID"
      Tab(0).ControlCount=   15
      TabCaption(1)   =   "History"
      TabPicture(1)   =   "frmDiscard.frx":001C
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "GridHistory"
      Tab(1).ControlCount=   1
      TabCaption(2)   =   "Store Stock"
      TabPicture(2)   =   "frmDiscard.frx":0038
      Tab(2).ControlEnabled=   -1  'True
      Tab(2).Control(0)=   "GridStore"
      Tab(2).Control(0).Enabled=   0   'False
      Tab(2).ControlCount=   1
      TabCaption(3)   =   "Total Stock"
      TabPicture(3)   =   "frmDiscard.frx":0054
...
Private Function CanIssue() As Boolean
    ...
    CanIssue = False
        If GridIssues.Rows <= 1 Then
            tr = MsgBox("There are no items to discard", vbCritical, "No items")
            dtcItem.SetFocus
            Exit Function
        End If
        If Not IsNumeric(dtcCheckedStaff.BoundText) Then
            tr = MsgBox("Please enter the staff member who checked the discard", vbInformation, "Checked by?")
...
Private Sub CheckConsume()
    ...
    With GridIssues
        ...
        For TemNum = 1 To GridIssues.Rows - 1
            ...
            Else
                With rsTemConsume
                    ...
                    temSql = "SELECT tblDiscard.ItemID, tblDiscard.Price, tblDiscard.BatchID, tblDiscard.StoreID, tblDiscard.StaffID, tblDiscard.CheckedStaffID, tblDiscard.Amount, tblDiscard.Date, tblDiscard.Time, tblDiscard.CategoryID " & _
                                "FROM tblDiscard "
        ...
        If NoError = True Then
            tr = MsgBox("All the items were successfully added as discarded", vbInformation, "Successful")
        Else
            tr = MsgBox("One or more items were NOT successfully added as discarded", vbCritical, "Error")
        End If
        .Visible = True
    End With
End Sub
...
Private Function ConsumeStocks(ByVal IStoreIDValue As Long, ByVal BatchIDValue As Long, ByVal Quentity As Double) As Boolean
    ...
    With rsTemBatch
        ...
        If !Stock < Quentity Then
            tr = MsgBox("There are no enough stocks in you store to discard", vbCritical, "No Enough Stocks")
            .Close
            Exit Function
        End If
...
eh:
    If .State = 1 Then
        ...
        .Close
    End If
    tr = MsgBox("Could not discard stocks from your store" & vbNewLine & Err.Description, vbCritical, "Error")
    Exit Function
    End With
End Function
...
With rsTemStore
    If .State = 1 Then .Close
    temSql = "SELECT tblDiscardCategory.DiscardCategory, tblDiscard.Date, tblDiscard.Amount, tblStaff.Name, tblDiscard.StoreID, * " & _
                "FROM tblDiscardCategory RIGHT JOIN (tblStaff RIGHT JOIN tblDiscard ON tblStaff.StaffID = tblDiscard.StaffID) ON tblDiscardCategory.DiscardCategoryID = tblDiscard.CategoryID " & _
                "Where (((tblDiscard.StoreID) = " & UserStoreID & ") And ((tblDiscard.ItemID) = " & dtcItem.BoundText & ")) " & _
                "ORDER BY tblDiscard.Date DESC"
    .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
    Dim i As Integer
    If .RecordCount > 0 Then
        While .EOF = False
            ...
            GridHistory.Text = !DiscardCategory
            GridHistory.Col = 1
...
With rsCCatogery
    If .State = 1 Then .Close
    temSql = "SELECT tblDiscardCategory.* FROM tblDIscardCategory ORDER BY tblDIscardCategory.DiscardCategory"
    .Open temSql, cnnStores, adOpenStatic, adLockReadOnly
End With
With dtcCCatogery
    Set .RowSource = rsCCatogery
    .ListField = "DIscardCategory"
    .BoundColumn = "DiscardCategoryID"
End With
...
Private Function CanExit() As Boolean
    ...
    Dim tr As Integer
    If GridIssues.Rows > 1 Then
        tr = MsgBox("There are " & (GridIssues.Rows - 1) & " items listed to be discarded, but the transction is not finalised. You have to delete all the items of confirn the consumption by clicking the issue button ", vbCritical, "Transaction NOT finalised")
        Me.ZOrder 0
        bttnIssue.SetFocus
        Exit Function
    End If
...
Private Function CanAdd() As Boolean
    ...
    If Not IsNumeric(dtcItem.BoundText) Then
        tr = MsgBox("You have not selected the item to discard", vbCritical, "Item?")
        dtcItem.SetFocus
        Exit Function
    End If
    If Not IsNumeric(dtcBatch.BoundText) Then
        tr = MsgBox("You have not selected a batch to discard", vbCritical, "Batch?")
        dtcBatch.SetFocus
        Exit Function
    End If
    If Not IsNumeric(txtQuentityByIssueUnit.Text) Or Not IsNumeric(txtQuentityByPurchaseUnit.Text) Then
        tr = MsgBox("You have not entered a valid quentity to discard", vbCritical, "Quentity?")
