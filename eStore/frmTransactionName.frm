...
Begin VB.Form frmTransactionName 
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   120
      Width           =   6615
      Begin VB.TextBox txtTransactionName 
         Height          =   360
         Left            =   1800
         TabIndex        =   2
         Top             =   720
   ...
   End
End
Attribute VB_Name = "frmTransactionName"
...
With rsViewTransaction
    ...
    .Open "Select* From tblMoveCatogery Order By MoveCatogery", cnnStores, adOpenStatic, adLockReadOnly
    If .RecordCount = 0 Then Exit Sub
    Set dtcTransaction.RowSource = rsViewTransaction
    dtcTransaction.BoundColumn = "MoveCatogeryID"
    dtcTransaction.ListField = "MoveCatogery"
...
On Error GoTo ErrorHandler
    ...
    If .State = 1 Then .Close
    .Open "Select tblMoveCatogery.* From tblMoveCatogery Where (TradenameID = " & TemTransactionID & ")", cnnStores, 3, 3
 
    
    If .RecordCount = 0 Then Exit Sub
    
    !MoveCatogery = Trim(txtTransactionName.Text)
    !IssueStore = Val(dtcIssueStores.BoundText)
    !ReceiveStore = Val(dtcReceiveStores.BoundText)
    .Update
...
On Error GoTo ErrorHandler
    ...
    If .State = 1 Then .Close
    .Open "Select tblMoveCatogery.* From tblMoveCatogery", cnnStores, adOpenStatic, adLockOptimistic
...
With rsTransaction
    ...
    If .State = 1 Then .Close
    .Open "Select tblMoveCatogery.* From tblMoveCatogery Where (MoveCatogeryID = " & dtcTransaction.BoundText & ")", cnnStores, adOpenStatic, adLockReadOnly
   
    
    If .RecordCount = 0 Then Exit Sub
    Call ClearValues
    
    If Not (!MoveCatogery) = "" Then txtTransactionName.Text = !MoveCatogery
    If Not (!IssueStore) = "" Then dtcIssueStores.BoundText = !IssueStore
    If Not (!ReceiveStore) = "" Then dtcReceiveStores.BoundText = !ReceiveStore
    
    TemTransactionID = !MoveCatogeryID
