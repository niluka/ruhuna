...
Begin VB.Form frmBatchStockExpired 
   ...
   End
End
Attribute VB_Name = "frmBatchStockExpired"
...
Private Sub FillGrid()
    ...
    With rsBatchStock
        ...
        temSelect = "SELECT tblItem.Display, tblBatch.Batch, tblBatch.DOE, tblBatchStock.Stock, tblCurrentPurchasePrice.PPrice*tblBatchStock.Stock AS StockValue"
        ...
        If IsNumeric(cmbCategory.BoundText) = False Then
            temWhere = "WHERE (((tblBatchStock.Stock)>0) AND ((tblBatch.DOE) <= '" & Format(dtpDOE.Value, "dd MMMM yyyy") & "'))"
        Else
            temWhere = "WHERE (((tblBatchStock.Stock)>0) AND ((tblBatch.DOE) <= '" & Format(dtpDOE.Value, "dd MMMM yyyy") & "') And (tblItem.ItemCategoryID = " & Val(cmbCategory.BoundText) & " ) )"
        End If
...
Private Sub btnExcel_Click()
    GridToExcel GridStock, "Batch Stock", cmbCategory.Text
End Sub
