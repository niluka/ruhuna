...
Begin VB.Form frmHospitalSale1 
   Caption         =   "Hospital Issue 1"
   ClientHeight    =   10470
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   15240
   ...
   Begin TabDlg.SSTab SSTab2 
      ...
      EndProperty
      TabCaption(0)   =   "Total"
      TabPicture(0)   =   "frmHospitalSale1.frx":0000
      ...
      TabPicture(1)   =   "frmHospitalSale1.frx":001C
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "frameCheque"
      Tab(1).Control(1)=   "frameCredit"
      Tab(1).Control(2)=   "frameCash"
      Tab(1).Control(3)=   "frameCreditCard"
      Tab(1).Control(4)=   "frameOutPatient"
      Tab(1).Control(5)=   "frameStaff"
      Tab(1).Control(6)=   "frameUnit"
      Tab(1).Control(7)=   "frameInPatient"
      Tab(1).ControlCount=   8
      TabCaption(2)   =   "Other"
      TabPicture(2)   =   "frmHospitalSale1.frx":0038
   ...
   End
End
Attribute VB_Name = "frmHospitalSale1"
...
'        End If
        ...
        ElseIf NewSale.Staff = True Then
            TemString = "SP"
        End If
        Printer.Print Tab(Tab1); "Issue No -    "; TemSaleBillID & "-" & TemString; "       Date : "; Format(dtpDate.Value, "dd MM yy"); Tab(Tab6); "Time : "; Time
        If NewSale.OutPatient = True Then
...
'        .Font = "Arial Black"
        ...
        Printer.Print
        Printer.Print Tab(Tab1); "Date : "; Format(dtpDate.Value, "dd MM yy")
...
Private Function ReceiveOther(SaleBillID As Long) As Long
    With rsTemCredit
        If .State = 1 Then .Close
        temSql = "SELECT * FROM tblReceivedOther"
        .Open temSql, cnnStores, adOpenStatic, adLockOptimistic
        .AddNew
        !ReceivedSTaffID = Val(dtcIssueStaff.BoundText)
        !ReceivedDate = dtpDate.Value
...
Private Function ReceiveCreditCard(SaleBillID As Long) As Long
    With rsTemCC
        ...
        !ReceivedDate = dtpDate.Value
        !ReceivedTime = Time
        !AuthrizationDate = dtpDate.Value
...
Private Sub CalculatePrice()
    If NewSale.Unit = True Then
        txtPrice.Text = Format(Val(txtCostRate.Text) * Val(txtQty.Text), "0.00")
