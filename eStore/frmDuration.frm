...
Option Explicit
    ...
    Dim rsViewDuration As New ADODB.Recordset
    Dim TemDurationID As Long
...
Private Sub dtcDuration_Click(Area As Integer)
    If IsNumeric(dtcDuration.BoundText) = False Then Exit Sub
...
Private Sub bttnAdd_Click()
    ClearValues
    AfterAdd
    txtDuration.Text = dtcDuration.Text
...
Private Sub bttnChange_Click()
    ...
    If Trim(txtDuration.Text) = "" Then NoName: Exit Sub
    With rsDuration
    On Error Resume Next
        If .State = 1 Then .Close
        .Open "Select* From tblDuration Where (DurationID = " & TemDurationID & ")", cnnStores, adOpenStatic, adLockOptimistic
        If .RecordCount > 0 Then
            !Duration = Trim(txtDuration.Text)
            !sFont = txtSFont.Text
...
Private Sub bttnSave_Click()
    ...
    'On Error Resume Next
        If .State = 1 Then .Close
        .Open "Select* From tblDuration", cnnStores, adOpenStatic, adLockOptimistic
        .AddNew
            !Duration = Trim(txtDuration.Text)
            !sFont = txtSFont.Text
            !Sfontsize = Val(txtSFontSize.Text)
...
Private Sub FillGenaricCombo()
    With rsViewDuration
        If .State = 1 Then .Close
        .Open "Select* From tblDuration Order By Duration", cnnStores, adOpenStatic, adLockReadOnly
        If .RecordCount = 0 Then Exit Sub
        Set dtcDuration.RowSource = rsViewDuration
        dtcDuration.ListField = "Duration"
        dtcDuration.BoundColumn = "DurationID"
    End With
End Sub
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
    If rsDuration.State = 1 Then rsDuration.Close: Set rsDuration = Nothing
    If rsViewDuration.State = 1 Then rsViewDuration.Close: Set rsViewDuration = Nothing
End Sub
...
Private Sub DisplaySelected()
    ...
    With rsDuration
        If .State = 1 Then .Close
        .Open "Select* From tblDuration Where (DurationID = " & dtcDuration.BoundText & ")", cnnStores, adOpenStatic, adLockReadOnly
        If .RecordCount = 0 Then Exit Sub
        Call ClearValues
        txtDuration.Text = !Duration
        If Not IsNull(!Comments) Then txtcomment.Text = !Comments
        TemDurationID = !DurationID
