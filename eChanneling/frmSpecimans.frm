...
Begin VB.Form frmSpeciman 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Speciman Names"
   ClientHeight    =   6840
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   11280
...
If Trim(txtName.Text) = "" Then
    TemResponce = MsgBox("You must enter a name for the speciman", vbCritical, "No Name")
    txtName.SetFocus
    Exit Sub
End If
...
With DataEnvironment1.rssqlTem
    ...
    .Source = "SELECT * from tblspecimans where speciman_id = " & TemSpecimanID
    If .State = 0 Then .Open
    If .RecordCount = 0 Then .Close: Exit Sub
    !speciman_ID = Trim(txtName.Text)
    .Update
    Grid1.Col = 1
    Grid1.Text = Trim(txtName.Text)
    Grid1.Col = 2
    Grid1.Text = !speciman_ID
    TemSpecimanID = !speciman_ID
End With
End Sub
...
With DataEnvironment1.rssqlTem
    If .State = 1 Then .Close
    .Source = "SELECT * from tblSpecimans order by speciman"
    If .State = 0 Then .Open
    If .RecordCount = 0 Then .Close: Exit Sub
    .MoveFirst
    NowRow = 0
...
Private Sub bttnDelete_Click()
    With DataEnvironment1.rssqlTem
        If .State = 1 Then .Close
        .Source = "SELECT speciman_ID from tblSpecimans where speciman_ID = " & TemSpecimanID
        If .State = 0 Then .Open
        If .RecordCount = 0 Then .Close:   Exit Sub
...
With DataEnvironment1.rssqlTem
    If .State = 1 Then .Close
    .Source = "Select * from tblSpecimans where ( speciman = '" & Trim(txtName.Text) & "')"
    If .State = 0 Then .Open
    If .RecordCount <> 0 Then
        TemResponce = MsgBox("The Speciman you entered already exist.", vbCritical, "Name Exists")
        txtName.SetFocus
        SendKeys "{home}+{end}"
        Exit Sub
    End If
...
With DataEnvironment1.rssqlTem
    If .State = 1 Then .Close
    .Source = "SELECT * from tblSpecimans "
    If .State = 0 Then .Open
    .AddNew
    !speciman = Trim(txtName.Text)
    .Update
    TemSpecimanID = !speciman_ID
...
With DataEnvironment1.rssqlTem1
    ...
    If Not IsNull(!speciman) Then txtName.Text = !speciman
    If .State = 1 Then .Close
End With
End Sub
