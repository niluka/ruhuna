...
Begin VB.Form FrmtodayEndShiftSummery 
   ...
   EndProperty
   Icon            =   "FrmToDayendShiftSummery.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Frame FrameShiftSummary 
      ...
      Top             =   1320
      Width           =   7815
      Begin btButtonEx.ButtonEx bttnDetailSummery 
         Height          =   375
         Left            =   5880
         TabIndex        =   32
         Top             =   3600
   ...
   End
End
Attribute VB_Name = "FrmtodayEndShiftSummery"
...
bttnDetailSummery.BackColor = BttnBackColour
bttnDetailSummery.ForeColor = BttnForeColour
'
...
FrmtodayEndShiftSummery.BackColor = FrameBackColour
FrmtodayEndShiftSummery.ForeColor = FrameForeColour
...
Private Sub bttnDetailSummery_Click()
...
Const PostSHape = "  ORDER BY tblPatientFacility.PaymentMode DESC , tblPatientFacility.PatientFacility_ID  }  AS cmmdPADF COMPUTE cmmdPADF, SUM(cmmdPADF.'PersonalDue') AS DocFee, SUM(cmmdPADF.'InstitutionDue') AS HosFee, SUM(cmmdPADF.'TotalDue') AS TotFee, ANY(cmmdPADF.'PaymentMode') AS PaymentMethodName BY 'PaymentMethod_Id'"

CSetPrinter.SetPrinterAsDefault (ReportPrinterName)

With DataEnvironment1

    If .rscmmdPADF_Grouping.State = 1 Then DataEnvironment1.rscmmdPADF_Grouping.Close
    
   
    Select Case SSTab1.Tab
    
    Case 0
    .Commands!cmmdPADF_Grouping.CommandText = PreSHape & Sql & " Where (BookingDate = '" & Date & "') " & PostSHape
    .cmmdPADF_Grouping
    
    Case 1
    .Commands!cmmdPADF_Grouping.CommandText = PreSHape & Sql & " Where (BookingDate ='" & DTPicker1.Value & "')" & PostSHape
    .cmmdPADF_Grouping
    
    Case 2
    .Commands!cmmdPADF_Grouping.CommandText = PreSHape & Sql & " Where (BookingDate Between '" & DTPicker2.Value & "' and '" & DTPicker3.Value & "')" & PostSHape
    .cmmdPADF_Grouping
    
    End Select
    
    If DataEnvironment1.rscmmdPADF_Grouping.RecordCount = 0 Then A = MsgBox("No Transaction to view", vbInformation + vbOKOnly, "No Transactions"): Exit Sub
    
    With DataReportPatientsDoctorsAgentsFacilities
    ...
    Set .DataSource = DataEnvironment1
        If HospitalDetails = True Then
            ...
            .Sections("ReportHeader").Controls.Item("rptLHeding3").Caption = "Cashier Day End Summary"
        Else
