...
Begin VB.Form frmCPeriodDoctorReport 
   ...
   EndProperty
   Icon            =   "frmCPeriodDoctorReport.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      EndProperty
      TabCaption(0)   =   "Today"
      TabPicture(0)   =   "frmCPeriodDoctorReport.frx":0442
      Tab(0).ControlEnabled=   0   'False
      Tab(0).Control(0)=   "lblDate"
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "Selected Day"
      TabPicture(1)   =   "frmCPeriodDoctorReport.frx":045E
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "DTPicker1"
      Tab(1).ControlCount=   1
      TabCaption(2)   =   "Period"
      TabPicture(2)   =   "frmCPeriodDoctorReport.frx":047A
      ...
      TabPicture(3)   =   "frmCPeriodDoctorReport.frx":0496
   ...
   End
End
Attribute VB_Name = "frmCPeriodDoctorReport"
...
Dim TemDoctorFee   As Double
    ...
    Case 0
    .Open "Select* From tblPatientFacility Where (Staff_ID = " & DtcDoctor.BoundText & " and FullyPaid = True AND PatientFacility_ID MOD " & IncomeDeflation & " = 1  and Cancelled = False and Refund = False and AppointmentDate = # " & Date & " #)  Order By PatientFacility_ID "
    
    Case 1
    .Open "Select* From tblPatientFacility Where (Staff_ID = " & DtcDoctor.BoundText & " and FullyPaid = True AND PatientFacility_ID MOD " & IncomeDeflation & " = 1  and Cancelled = False and Refund = False and AppointmentDate = # " & DTPicker1.Value & " #)  Order By PatientFacility_ID "
    
    Case 2
    .Open "Select* From tblPatientFacility Where (Staff_ID = " & DtcDoctor.BoundText & " and FullyPaid = True AND PatientFacility_ID MOD " & IncomeDeflation & " = 1  and Cancelled = False and Refund = False and AppointmentDate Between  # " & DTPicker2.Value & " # and # " & DTPicker3.Value & " #)  Order By PatientFacility_ID "
...
Const Sql = "SELECT tblTitle.Title , tblDoctor.DoctorName, tblPatientFacility.* FROM ((tblDoctor LEFT OUTER JOIN tblTitle ON tblDoctor.DoctorTitle_ID = tblTitle.Title_ID) RIGHT OUTER JOIN tblPatientFacility ON tblDoctor.Doctor_ID = tblPatientFacility.Staff_ID)"
Const PostSHape = "}  AS cmdDoctorIncomeC COMPUTE cmdDoctorIncomeC, SUM(cmdDoctorIncomeC.'PersonalFee') AS TotalPersonalFee, ANY(cmdDoctorIncomeC.'Title') AS TitalName, COUNT(cmdDoctorIncomeC.'PersonalFee') AS TotalPatients BY 'DoctorName','PersonalFee'"
...
With DataEnvironment1
    ...
    If PayToDoctor = True Then
        .Commands!cmdDoctorIncomeC_Grouping.CommandText = pershape & Sql & " Where (FullyPaid = True) AND PatientFacility_ID MOD " & IncomeDeflation & " = 1  and (Cancelled = False) and (Refund = False) Order By DoctorName" & PostSHape
    Else
        .Commands!cmdDoctorIncomeC_Grouping.CommandText = pershape & Sql & " Where (FullyPaid = True) AND PatientFacility_ID MOD " & IncomeDeflation & " = 1  and (Cancelled = False) and (Refund = False) and (patientabsent = false) Order By DoctorName" & PostSHape
    End If
...
With DataEnvironment1.rssqlTem12
    ...
    Case 0
    .Open " SELECT tblPatientFacility.*, tblPatientMainDetails.FirstName, tblPatientFacility.PatientFacility_ID FROM tblPatientMainDetails INNER JOIN tblPatientFacility ON tblPatientMainDetails.Patient_ID = tblPatientFacility.PatientID Where (Staff_ID = " & DtcDoctor.BoundText & " and FullyPaid = True AND PatientFacility_ID MOD " & IncomeDeflation & " = 1  and Cancelled = False and Refund = False and AppointmentDate = # " & Date & " #) ORDER BY tblPatientFacility.PatientFacility_ID"

    
    Case 1
    .Open " SELECT tblPatientFacility.*, tblPatientMainDetails.FirstName, tblPatientFacility.PatientFacility_ID FROM tblPatientMainDetails INNER JOIN tblPatientFacility ON tblPatientMainDetails.Patient_ID = tblPatientFacility.PatientID Where (Staff_ID = " & DtcDoctor.BoundText & " and FullyPaid = True AND PatientFacility_ID MOD " & IncomeDeflation & " = 1  and Cancelled = False and Refund = False and AppointmentDate = # " & DTPicker1.Value & " #) ORDER BY tblPatientFacility.PatientFacility_ID"

    Case 2
    
    .Open " SELECT tblPatientFacility.*, tblPatientMainDetails.FirstName, tblPatientFacility.PatientFacility_ID FROM tblPatientMainDetails INNER JOIN tblPatientFacility ON tblPatientMainDetails.Patient_ID = tblPatientFacility.PatientID Where (Staff_ID = " & DtcDoctor.BoundText & " and FullyPaid = True AND PatientFacility_ID MOD " & IncomeDeflation & " = 1  and Cancelled = False and Refund = False and AppointmentDate Between  # " & DTPicker2.Value & " # and # " & DTPicker3.Value & " #) ORDER BY tblPatientFacility.PatientFacility_ID"
    Case 3
    Call PrintThismonthDoctorIncome
    End Select
...
With DataEnvironment1
    ...
    .Commands!cnmdDoctorIncome_Grouping.CommandText = PreSHape & Sql & " Where appointmentdate Between  #" & firstDay & "# and #" & LastDay & "# and FullyPaid = True AND PatientFacility_ID MOD " & IncomeDeflation & " = 1  and Refund = False and Cancelled = False " & PostSHape
